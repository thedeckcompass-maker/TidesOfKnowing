<!--
  EDITORIAL EXTRACTION ENVELOPE
  Card: The Fool
  Extracted: 2026-07-06
  Status: EXTRACTED (editorial test)
  Source: src/content/repeating-card-meanings/majors/the-fool.md
  Do not edit the PRODUCTION SOURCE COPY section below until reinsertion workflow begins.
-->

# Editorial Reconstruction Envelope

## Identifiers

| Field | Value |
|-------|-------|
| collection_id | `majors/the-fool` |
| url_slug | `the-fool` |
| production_source_path | `src/content/repeating-card-meanings/majors/the-fool.md` |
| arcana | major |
| suit | n/a |
| card_number | 0 |
| tier | full |
| status (frontmatter) | draft |
| ready (frontmatter) | not set (auto-readiness applies) |

## URLs and routing

| Surface | Path |
|---------|------|
| canonical_entity_page | `/repeating-card-meanings/the-fool/` |
| seo_hub | `/repeating-card-meanings/` |
| tool_selector | `/tools/repeating-card-meanings/?card=majors/the-fool#rcm-card-select` |
| tool_deep_link | `/tools/repeating-card-meanings/majors/the-fool/` |
| frontmatter slug | `repeating-card-meanings/the-fool` |
| frontmatter canonicalUrl | `/repeating-card-meanings/the-fool/` |

## Image

| Field | Value |
|-------|-------|
| openGraphImage (frontmatter) | `/images/tarot/rws/the-fool.jpg` |
| resolved_image_src | `/images/tarot/rws/the-fool.jpg` |
| image_file | `public/images/tarot/rws/the-fool.jpg` |
| image_alt (computed at render) | `The Fool tarot card` |

## Computed display values (not stored in markdown)

These are generated at build/render time. Do not add to production frontmatter unless explicitly part of reinsertion.

| Field | Computed value |
|-------|----------------|
| displayTitle | `The Fool` |
| panelDomId | `majors--the-fool` |
| metaTitle (no metaTitle in frontmatter) | `Repeating The Fool Meaning \| Tarot Pattern Interpretation` |
| metaDescription (from summary) | `When The Fool keeps returning, the Seeker is often living at a threshold: something is ready to begin, change, or be trusted into, yet movement or commitment remains incomplete. Repetition may point to hesitation before the unknown, cycles of starting without staying, or a wound that made openness feel unsafe. The card is less about naivety than about the real question of whether life can be entered without guarantees. Fool periods ask what beginning is being approached, avoided, or repeated without depth. (Major Arcana)` |
| author_attribution (entity pages) | `By Leigh Spencer, fourth-generation Matakite (seer), founder of The COMPASS Method™, 40+ years tarot experience and 30 years in journalism.` |
| metaAuthor (BaseLayout) | `Leigh Spencer` |
| eyebrow (entity variant) | `Canonical repeating card reference` |
| showFeaturedSnippet | true (featuredSnippetAnswer is distinct from panel summary) |

## Breadcrumbs (entity page)

1. Home → `/`
2. Repeating Card Meanings → `/repeating-card-meanings/`
3. The Fool → (current page)

## Breadcrumbs (tool deep-link)

1. Home → `/`
2. Tools → `/tools/`
3. Repeating Card Meanings → `/tools/repeating-card-meanings/`
4. Major Arcana → `/tools/repeating-card-meanings/majors/`
5. The Fool → (current page)

## Section navigation anchors (H2, scoped)

Anchor prefix: `majors--the-fool--`

| Nav label | Anchor id |
|-----------|-----------|
| Core Repeating Message | `majors--the-fool--core-repeating-message` |
| When This Card Repeats Weekly | `majors--the-fool--when-this-card-repeats-weekly` |
| When This Card Repeats Monthly | `majors--the-fool--when-this-card-repeats-monthly` |
| When This Card Repeats Seasonally | `majors--the-fool--when-this-card-repeats-seasonally` |
| When This Card Repeats Across Years | `majors--the-fool--when-this-card-repeats-across-years` |
| Love & Relationships | `majors--the-fool--love-relationships` |
| Career & Purpose | `majors--the-fool--career-purpose` |
| Money & Stability | `majors--the-fool--money-stability` |
| Spiritual Growth | `majors--the-fool--spiritual-growth` |
| Emotional & Mental Patterns | `majors--the-fool--emotional-mental-patterns` |
| Family & Generational Dynamics | `majors--the-fool--family-generational-dynamics` |
| Health & Energy | `majors--the-fool--health-energy` |
| The Shadow Expression | `majors--the-fool--the-shadow-expression` |
| The Integrated Expression | `majors--the-fool--the-integrated-expression` |
| Why This Energy Has Not Released Yet | `majors--the-fool--why-this-energy-has-not-released-yet` |
| What This Card Wants the Seeker to Understand | `majors--the-fool--what-this-card-wants-the-seeker-to-understand` |
| Signs the Pattern Is Beginning to Resolve | `majors--the-fool--signs-the-pattern-is-beginning-to-resolve` |
| Reflective Questions | `majors--the-fool--reflective-questions` |
| Practical Integration Actions | `majors--the-fool--practical-integration-actions` |

Note: H1 (`# The Fool Repeating Meaning`) is stripped from rendered HTML. H1-equivalent display uses `displayTitle` in page header.

## Generated FAQ block (entity pages only)

Built from frontmatter by `buildRepeatingCardQuestions()`.

| # | Question | Answer source |
|---|----------|---------------|
| 1 | What does it mean when The Fool keeps appearing? | `featuredSnippetAnswer` |
| 2 | What is the deeper pattern behind repeating The Fool? | `answerEngineSummary` |
| 3 | Why does the fool keep showing up in tarot? | `featuredSnippetAnswer` |

## Related cards (governed map, not in frontmatter)

Source: `src/data/repeating-card-related-map.ts` (compiled from `seo-aeo-output/03-related-cards/related-card-map.yaml`)

| collection_id | relationship_type |
|---------------|-------------------|
| majors/the-world | progressive |
| majors/the-magician | progressive |
| wands/ace-of-wands | same-theme |
| cups/ace-of-cups | same-theme |
| cups/eight-of-cups | shadow-pair |

## Taxonomy (planning artifact, NOT in production frontmatter)

Source: `src/content/repeating-card-meanings/seo-aeo-output/02-taxonomy/card-taxonomy-map.yaml`
...
## Internal linking metadata (planning artifact)

Source: `src/content/repeating-card-meanings/seo-aeo-output/06-internal-linking/internal-linking-map.yaml`

```yaml
clusters: [1, 10]
linking_tier: tier-2
hub_status: false
anchor_text_suggestion: "The Fool repeating meaning"
anchor_text_alt: "when The Fool keeps appearing"
pillar_page_candidates:
  - "When Tarot Keeps Showing You a Beginning Card"
  - "When Tarot Keeps Showing You Identity or Self-Worth Cards"
```

## Metadata map entry (planning artifact, differs from live frontmatter)

Source: `src/content/repeating-card-meanings/seo-aeo-output/04-metadata/card-metadata-map.yaml`

Note: `metaTitle` and `metaDescription` below are NOT in production frontmatter. Live page uses computed fallbacks.

```yaml
metaTitle: "The Fool Keeps Appearing in Tarot | Tides of Knowing"
metaDescription: "When the Fool keeps appearing, it marks a threshold that returns uncrossed — every beginning stays a beginning. The repeating pattern asks what is keeping you at the edge."
```

## Footer links (hardcoded on entity page)

- Repeating card pattern library → `/repeating-card-meanings/`
- Card selector tool → `/tools/repeating-card-meanings/`
- The COMPASS Method™ → `/articles/compass-method/`

## Frontmatter fields absent (schema defaults apply)
...
```
src/content/repeating-card-meanings/majors/the-fool.md
  → Astro collection: repeatingCardMeanings
  → isRepeatingMeaningReady()
  → buildRepeatingCardPageModel()
  → RepeatingCardMeaningArticle (variant: entity)
  → BaseLayout (SEO, JSON-LD)
```

Key lib files: `repeatingCardPageModel.ts`, `repeatingCardSeo.ts`, `repeatingCardEntitySchema.ts`, `repeatingCardQuestions.ts`, `repeatingCardRelated.ts`, `rcmHeadingIds.ts`, `tarotCardImage.ts`

---

# PRODUCTION SOURCE COPY

The section below is an exact copy of `src/content/repeating-card-meanings/majors/the-fool.md` as of 2026-07-06. Do not edit during external editorial review.
