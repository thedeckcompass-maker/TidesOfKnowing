...
| Feature preview | Deployed; auth recheck pending | The Pages preview for `a0ccfc2` deployed on the stable branch alias. After the test webhook secret was saved and Preview retried, an unsigned POST returned `Missing Stripe signature`, confirming that deployment sees the webhook secret. The owner has since saved the staging Supabase URL and publishable key in Preview; a 23 September request to `/auth/register/` still returned `Practice Commons sign-in is not configured yet`. Redeploy Preview and recheck before sign-in. |
...
| Stripe test-mode lifecycle | Blocked on staging service key and creator sign-in | All three sandbox prices, the test Stripe key and a six-event webhook destination have been configured for Preview. After the Supabase bindings take effect, verify that Preview also has the staging project's server-only `SUPABASE_SERVICE_ROLE_KEY` before initiating checkout. Do not use a production Supabase key. Then exercise the real event lifecycle and inspect database effects. Keep API keys and webhook signing secrets out of Git and chat. |
