...
| Feature preview | Sign-in bindings active; redirect allowlist missing | The staging Supabase URL and publishable key are active: an invalid sign-in request returns normal email validation (HTTP 400). An unsigned webhook POST returns `Missing Stripe signature` (HTTP 400), confirming the test webhook secret is active. The owner reports saving the staging service key in Preview, but its use has not been verified. The staging Supabase project currently has Site URL `http://localhost:3000` and no allowed Redirect URLs, so email-link sign-in cannot reliably return to the Preview callback. Add the branch callback to the staging allowlist after explicit approval, then test sign-in. |
...
| Stripe test-mode lifecycle | Blocked on staging auth redirect and signed-in checkout | All three sandbox prices, the test Stripe key and a six-event webhook destination have been configured for Preview. The owner reports adding the staging project's server-only `SUPABASE_SERVICE_ROLE_KEY` to Preview. Once the auth callback is allowed, exercise the real event lifecycle and inspect database effects; an unsigned webhook probe does not verify the server key. Keep API keys and webhook signing secrets out of Git and chat. |
