...
| Feature preview | Staging sign-in bindings active; server key recheck pending | The Pages preview for `df12ed4` deployed on the stable branch alias. The staging Supabase URL and publishable key are active: an invalid sign-in request returns normal email validation (HTTP 400). An unsigned webhook POST returns `Missing Stripe signature` (HTTP 400), confirming the test webhook secret is active. The owner reports saving the staging service key in Preview; redeploy and verify its use in a controlled sandbox checkout. |
...
| Stripe test-mode lifecycle | Pending sandbox checkout and signed events | All three sandbox prices, the test Stripe key and a six-event webhook destination have been configured for Preview. The owner reports adding the staging project's server-only `SUPABASE_SERVICE_ROLE_KEY` to Preview. After redeployment, exercise the real event lifecycle and inspect database effects; a successful unsigned webhook probe does not verify the server key. Keep API keys and webhook signing secrets out of Git and chat. |
