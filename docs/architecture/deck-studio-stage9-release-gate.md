...
| Feature preview | Staging email-link sign-in passed; creator journey pending | The staging Supabase URL and publishable key are active: an invalid sign-in request returns normal email validation (HTTP 400). An unsigned webhook POST returns `Missing Stripe signature` (HTTP 400), confirming the test webhook secret is active. The Preview app completed a server-only OTP claim and created the active test profile, which verifies its staging service key can call the auth RPC and write the profile. The explicitly approved `https://feature-deck-creator-studio-m3g9.tidesofknowing.pages.dev/auth/callback/**` is in the staging Supabase Redirect URLs. The owner reports opening the delivered link and signing in; inspect the enrolment page in that browser before checkout. |
