...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (eventError?.code === "23505") return json({ ok: true, idempotent: true });
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      await service.from("studio_checkout_reservations").update({ status: "completed", stripe_checkout_session_id: session.id }).eq("id", reservationId).eq("user_id", userId);
      await service.from("studio_entitlement_audit").insert({ user_id: userId, action: "checkout_completed", plan_code: planCode, detail: { stripe_event_id: event.id, stripe_session_id: session.id } });
      await service.from("studio_analytics_events").insert({ event_name: "purchase", user_id: userId, plan_code: planCode, source: "stripe_test_webhook" });
      ...
      await service.from("studio_entitlement_audit").insert({ user_id: userId, action: event.type, plan_code: planCode, detail: { stripe_event_id: event.id, status } });
    } else if (event.type === "invoice.paid" || event.type === "invoice.payment_failed") {
      const invoice = event.data.object as Stripe.Invoice;
      const subId = subscriptionId(invoice.parent?.subscription_details?.subscription);
      if (subId) {
        if (event.type === "invoice.paid") {
          const { data: entitlement } = await service.from("studio_entitlements").select("user_id, plan_code, paid_invoice_count").eq("stripe_subscription_id", subId).maybeSingle();
          if (entitlement) {
            ...
            await service.from("studio_entitlements").update({ status: "active", access_mode: "write", paid_invoice_count: paidInvoiceCount }).eq("user_id", entitlement.user_id);
            await service.from("studio_entitlement_audit").insert({ user_id: entitlement.user_id, action: "invoice_paid", plan_code: entitlement.plan_code, detail: { stripe_event_id: event.id, invoice_id: invoice.id, paid_invoice_count: paidInvoiceCount, cancel_at_period_end: completesCircle } });
          }
        } else {
          const { data: entitlement } = await service.from("studio_entitlements").select("user_id, plan_code").eq("stripe_subscription_id", subId).maybeSingle();
          if (entitlement) {
            await service.from("studio_entitlements").update({ status: "past_due", access_mode: "read_only", read_only_until: null }).eq("user_id", entitlement.user_id);
            await service.from("studio_entitlement_audit").insert({ user_id: entitlement.user_id, action: "invoice_payment_failed", plan_code: entitlement.plan_code, detail: { stripe_event_id: event.id, invoice_id: invoice.id } });
          }
        }
      }
    }
    await service.from("studio_billing_events").update({ outcome: "completed", processed_at: new Date().toISOString() }).eq("stripe_event_id", event.id);
    return json({ ok: true });
  } catch (error) {
