...
function baselineDateFromPath(filePath) {
  const match = path.basename(filePath).match(/gsc-(?:not-indexed|discovered-not-indexed-urls)-(\d{4}-\d{2}-\d{2})/i);
  return match?.[1] ?? new Date().toISOString().slice(0, 10);
}

function isDiscoveredUrlExport(filePath) {
  return /discovered-not-indexed-urls/i.test(path.basename(filePath));
}

function baselineReportBasename(filePath, baselineDate) {
  return isDiscoveredUrlExport(filePath)
    ? `gsc-discovered-baseline-${baselineDate}`
    : `gsc-baseline-${baselineDate}`;
}
...
function writeSummaryBaselineReport({
  ...
  const outPath = path.join(BASELINES_DIR, `${baselineReportBasename(exportPath, baselineDate)}.md`);
...
function writeBaselineReport({
  ...
  const discovered = isDiscoveredUrlExport(exportPath);

  const lines = [
    discovered
      ? `# GSC Discovered – not indexed URL baseline — ${baselineDate}`
      : `# GSC not-indexed baseline — ${baselineDate}`,
    "",
    "Generated by `scripts/analyze-repeating-card-gsc-export.mjs`.",
    "",
  ...
  if (discovered && entityInExport.length >= 70) {
    lines.push(
      `- **Discovered bucket is entity-heavy:** ${entityInExport.length} of 78 canonical entity URLs are in this export — queueing/indexing lag after cluster launch, not tool-canonical noise (0 tool deep-links here).`,
    );
  }
  if (discovered && (counts.tag ?? 0) >= 30) {
    lines.push(
      `- **Tag noise:** ${counts.tag ?? 0} tag URLs in Discovered — expected low-priority indexing; separate from entity strategy.`,
    );
  }
  if (entityInExport.length > 40 && !discovered) {
    lines.push(
  ...
  if (entityInExport.length === 0) {
    ...
    if (discovered) {
      const slugs = entityInExport
        ...
        .filter(Boolean)
        .sort();
      lines.push("", "### Entity slugs in Discovered export", "", slugs.join(", "), "");
    }
  ...
  if (discovered && entityMissing.length > 0 && entityMissing.length <= 5) {
    lines.push("", "### Entities not in Discovered export", "");
    for (const url of entityMissing) {
...
function maybeUpdateTracker(entityUrlsInExport, gscByUrl, baselineDate, exportPath) {
  if (!fs.existsSync(TRACKER)) {
    console.warn(`Tracker not found: ${TRACKER}`);
    return;
  }
  const discoveredExport = isDiscoveredUrlExport(exportPath);
  const raw = fs.readFileSync(TRACKER, "utf8");
  const lines = raw.replace(/^\uFEFF/, "").split(/\r?\n/).filter(Boolean);
  const header = lines[0];
  ...
  for (const line of lines.slice(1)) {
    ...
    if (discoveredExport) {
      cols[3] = inExport ? "N" : cols[3] || "";
      cols[4] = baselineDate ?? today;
      cols[5] = inExport
        ? "Discovered – currently not indexed"
        : "not_in_discovered_export";
      cols[9] = inExport
        ? `Discovered URL export ${baselineDate}`
        : `Absent from Discovered export ${baselineDate} (may be indexed or other bucket)`;
    } else {
...
if (updateTracker) {
  ...
  maybeUpdateTracker(entityUrlsInExport, gscByUrl, baselineDate, exportPath);
}
